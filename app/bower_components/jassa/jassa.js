...
(function() {
    ...
    ns.GeoMapFactory = Class.create({
        ...
        createMapForBounds: function(bounds) {
            ...
            var result = new ns.Mapping(null, pattern, new sparql.ElementFactoryConst(element));
            return result;
        }
    });
...
(function() {
    ...
    ns.GeoExprUtils = {
        ...
        createExprWgs84Intersects: function(varX, varY, bounds, castNode) {
            var lon = new Jassa.sparql.ExprVar(varX);
            var lat = new Jassa.sparql.ExprVar(varY);
            ...
            var xMin = Jassa.sparql.NodeValue.makeDecimal(bounds.left);
            var xMax = Jassa.sparql.NodeValue.makeDecimal(bounds.right);
            var yMin = Jassa.sparql.NodeValue.makeDecimal(bounds.bottom);
            var yMax = Jassa.sparql.NodeValue.makeDecimal(bounds.top);
...
(function($) {
    ...
    ns.QuadTreeCache = Class.create({ 
        ...
        createFlowForBounds: function(bounds) {
            var store = new Jassa.sponate.StoreFacade(this.sparqlService); //, prefixes); 
